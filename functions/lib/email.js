...
exports.sendAdminNewApplicationNotification = sendAdminNewApplicationNotification;
exports.sendInstructorApprovalEmail = sendInstructorApprovalEmail;
exports.sendInstructorRejectionEmail = sendInstructorRejectionEmail;
...
async function sendAdminNewApplicationNotification(instructorName, instructorEmail, affiliation) {
    await sendEmail(ADMIN_EMAIL, "New Instructor Application - Newsvendor Game", `
...
async function sendInstructorApprovalEmail(email, name) {
    await sendEmail(email, "Your Instructor Account Has Been Approved - Newsvendor Game", `
...
async function sendInstructorRejectionEmail(email, name, reason) {
    await sendEmail(email, "Instructor Application Update - Newsvendor Game", `
...
async function sendInstructorRevocationEmail(email, name, reason) {
    await sendEmail(email, "Instructor Access Revoked - Newsvendor Game", `
...
async function sendAdminPendingApprovalsSummary(pendingCount, pendingInstructors) {
    const instructorRows = pendingInstructors
      ...
      `)
        .join("");
    await sendEmail(ADMIN_EMAIL, `${pendingCount} Instructor Application${pendingCount === 1 ? "" : "s"} Pending - Newsvendor Game`, `
