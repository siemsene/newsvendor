...
exports.invNormCdf = invNormCdf;
exports.optimalOrderQuantity = optimalOrderQuantity;
exports.generateDemandDataset = generateDemandDataset;
...
function truncatedNormalInt(mu, sigma, rng) {
...
function optimalOrderQuantity(mu, sigma, price, cost, salvage) {
    const cu = price - cost;
...
function profitForSeries(demands, Q, price, cost, salvage) {
    let profit = 0;
...
function bestOrderQuantityForSeries(demands, params) {
...
function candidateOk(training, inGame, params, optimalQ, tolerances) {
    ...
    return (Math.abs(mT - params.mu) <= tolerances.tolMean &&
...
function scoreDataset(training, inGame, params, optimalQ) {
    const mT = mean(training), sT = std(training);
    const mG = mean(inGame), sG = std(inGame);
    const skT = skewness(training), skG = skewness(inGame);
    const kT = excessKurtosis(training), kG = excessKurtosis(inGame);
    const mu = params.mu, sigma = params.sigma;
    const repPenalty = Math.abs(mT - mu) / Math.max(1e-9, sigma) +
...
function generateDemandDataset(params, seed) {
    ...
    for (let it = 0; it < iterations; it++) {
        ...
        while (draws.length < nTotal)
            draws.push(truncatedNormalInt(params.mu, params.sigma, rng));
        // shuffle
